...
class WebDavRepository(
    ...
    suspend fun testConnection(account: WebDavAccount): WebDavTestResult =
        withContext(Dispatchers.IO) {
            ...
            val resolvedUrl = client.getEffectiveBaseUrl()
            val probeInfo = client.probe()
            try {
                val files = client.listRoot()
                val note = if (resolvedUrl.trimEnd('/') != account.url.trimEnd('/')) " [解析到: $resolvedUrl]" else ""
                WebDavTestResult(true, "OK, 共 ${files.size} 项$note ($probeInfo)", resolvedUrl)
            } catch (e: Exception) {
                ...
                if (ok) {
                    ...
                    WebDavTestResult(false, "${e.message ?: e.javaClass.simpleName} | $probeInfo", resolvedUrl)
                }
            }
        }
